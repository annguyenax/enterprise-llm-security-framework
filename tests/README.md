...
| `test_benchmark_v2_freeze.py` | **Phase 12D, updated across two Code X audit-resolution rounds and the resumed completion pass.** Candidate-manifest correctness covers all 9 policy-bearing files, SHA-256/size/path/order safety, mutation detection for all five artifact kinds, missing/new files, and deterministic restoration. Fresh freeze now also has regression coverage proving it refuses a tree missing either exemptions or authoring provenance. |
...
Latest Phase 12D evidence (round 3, malformed-value re-audit): `246 passed`
across the three benchmark modules; complete repository suite `569 passed,
1 warning`, with no ignored test module.
