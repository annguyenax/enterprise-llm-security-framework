...
## 6. Cập nhật tiến độ Phase 5 (bổ sung 2026-07-11)

Đã hiện thực RAG Context Guard (`app/guards/rag_guard.py`, 8 luật rule-based) và pipeline ingestion dữ liệu tổng hợp (`app/services/dataset_loader.py`, đọc `datasets/clean/` và `datasets/poisoned/`, chunking xác định-được (deterministic), không dùng vector database hay embedding thật). Đã thêm endpoint `POST /v1/guard/rag-context` và tích hợp guard này vào `POST /v1/gateway/chat`. Toàn bộ luật đã được kiểm chứng trực tiếp trên 10 tài liệu benchmark thật (5 sạch, 5 nhiễm độc) của Phase 3.1, khớp đúng `expected_guard_decision` đã đóng băng trước đó (trừ một trường hợp lệch có chủ đích và đã ghi chú lý do trong code). Chưa triển khai: vector database thật, embedding/similarity search thật, và lệnh gọi LLM thật — các phần này vẫn thuộc các phase sau (LLM Provider Adapter, Phase 5 hàng còn lại; Evaluation Runner, Phase 7).
