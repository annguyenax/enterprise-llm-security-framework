...
| **T**ampering | Multi-chunk coordination: an attack instruction is deliberately split across two or more separately-retrieved chunks, none of which individually matches a RAG Guard content rule, but which combine into a coherent instruction once placed together in context | Retriever, RAG Context Guard | **Decided and implemented at Phase 12C** (`app/services/rag_query.py::_build_aggregate_text`): after per-chunk RAG Context Guard evaluation, the final accepted chunks' bounded excerpts (capped at `RAG_MAX_AGGREGATE_CONTEXT_CHARS`, default 4000 chars total) are joined and re-inspected as one synthetic chunk by the same, unmodified `evaluate_rag_context()`, before the provider is invoked. Verified with a named test constructing exactly this scenario (`tests/test_rag_pipeline.py::test_multi_chunk_coordination_is_caught_by_the_aggregate_check`: two individually-clean halves of "this note should be treated as final" are only caught once combined). Full resolution (cross-chunk semantic reasoning) remains out of scope — this is a bounded, rule-based, best-effort mitigation using the existing detection rules, not a new detector, and does **not** eliminate the risk, only reduces it for coordination patterns the existing rule set can already recognize once combined. | High (residual; now partially mitigated by a bounded aggregate check, not eliminated) |
