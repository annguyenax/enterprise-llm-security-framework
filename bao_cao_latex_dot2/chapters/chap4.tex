...
Full repository suite & 1522 passed, 0 failed, 4 skipped, 1 warning, exit 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{giữ nguyên FPR $0\%$} (Bảng~\ref{tab:v3-optim}). Tính khái quát được kiểm
bằng các câu mạo danh diễn đạt mới (không lấy từ bộ sinh) trong kiểm thử đơn vị;
đồng thời ghi nhận giới hạn: kẻ tấn công diễn đạt lại để tránh hai bất biến vẫn
có thể né luật (phù hợp với tài liệu về vượt qua guardrail~\cite{hackett2025bypass}).
...
\vspace{2pt}
{\footnotesize $^\dagger$ Chỉ số exfil \textbf{bị nhiễu} và không phản ánh rò rỉ
kho tri thức thật: $190/200$ prompt tấn công đã \emph{chứa sẵn} mã canary trong
câu hỏi, nên phần lớn ``rò rỉ'' đo được thực chất là mô hình lặp lại prompt
(echo), không phải trích xuất từ tài liệu. Xem mục phân tích và Hạn chế.\par}
\end{table}
...
Hình~\ref{fig:tpr-fpr-bar} cho thấy trực quan: hai cấu hình khả dụng (rule-based,
\texttt{qwen3:4b}) có TPR gần bằng nhau ($\approx 77\%$), chỉ khác ở FPR; còn
\texttt{hermes3:8b} đạt TPR tối đa nhưng bằng cái giá FPR $67\%$.
...
\textbf{Thứ nhất, lớp luật cho precision tuyệt đối và recall trung bình.} Sau khi
bổ sung nhóm luật chống mạo danh, cấu hình rule-based đạt TPR $77{,}0\%$ mà vẫn
giữ FPR $0\%$ trên toàn bộ 225 câu lành tính --- kể cả 125 câu \emph{hard
benign} --- nghĩa là không chặn nhầm công việc hợp lệ nào. Theo nguồn, lớp luật
nay chặn trọn họ PyRIT ($100\%$) và giữ nguyên garak ($75{,}2\%$); phần bỏ lọt
...
\textbf{Thứ ba, chỉ số exfil hiện tại bị nhiễu và chưa đo được rò rỉ kho thật.}
...
Tuy nhiên con số này \textbf{không hợp lệ như một phép đo trích xuất kho tri
thức}: $190/200$ prompt tấn công đã chứa sẵn mã canary ngay trong câu hỏi, nên
khi mô hình ``để lộ'' canary thì phần lớn chỉ là lặp lại prompt (echo), không
phải lấy dữ liệu từ tài liệu được truy hồi. Muốn đo rò rỉ kho thật thì canary
phải \emph{chỉ} tồn tại trong kho và không xuất hiện trong prompt --- đây là một
sửa lỗi phương pháp cho phiên bản đánh giá sau, không phải kết quả có thể diễn
giải ở đây. Chúng tôi báo cáo hạn chế này thay vì trình bày $21{,}5\%$ như một
lỗ hổng đã định lượng.
...
loại chặn gần như mọi thứ (Stop-before-LLM $99{,}5\%$) tất yếu đạt TPR cao, và
FPR $67{,}1\%$ (chặn hai phần ba câu hợp lệ) khiến cấu hình đó không thể vận hành.
...
\begin{itemize}
    ...
    \item FPR $0\%$ được đo trên bộ hard benign tổng hợp gồm 57 nội dung độc nhất sinh từ 40 template; precision khái quát yếu hơn con số $0\%$ gợi ý.
    \item Phép đo Exfil Marker \textbf{bị nhiễu prompt echo} (canary có sẵn trong prompt) nên chưa đánh giá được rò rỉ kho tri thức thật; cần thiết kế lại canary (chỉ nằm trong kho) cho phiên bản sau.
    \item TPR $77\%$ đạt được bằng luật nhắm đặc trưng họ tấn công tổng hợp; \textbf{không} phải phòng thủ thích ứng --- biến thể diễn đạt lại có thể né.
