...
Full repository suite & 1524 passed, 0 failed, 4 skipped, exit 0 \\
\bottomrule
\end{tabular}
\end{table}
...
lõi (precision hoàn hảo trên bộ tổng hợp). Thêm \textbf{Semantic Judge}
...
\section{Demo A/B thời gian thực: có tường so với không tường}
\label{sec:ab-demo}

Ngoài đo lường định lượng ở trên, nhóm thực hiện một phiên demo thủ công trên kho
tri thức doanh nghiệp tổng hợp (kho tài liệu HR/IT có phân quyền RBAC), chạy
\emph{cùng một prompt} trên hai luồng chat: \textbf{không tường} (gọi thẳng mô
hình, chỉ còn cơ chế tự từ chối của mô hình) và \textbf{có tường} (đi qua chuỗi
guard). Mục tiêu là minh hoạ trực quan điều số liệu đã chỉ ra --- một LLM thông
thường \emph{có thể bị thao túng và làm lộ dữ liệu} khi thiếu lớp guard bảo vệ.

\begin{table}[H]
\centering
\caption{Đối chiếu A/B trên một số kịch bản demo (kho tri thức có RBAC)}
\label{tab:ab-demo}
\begin{tabularx}{\linewidth}{@{}p{0.34\linewidth}YY@{}}
\toprule
\textbf{Kịch bản (cùng prompt)} & \textbf{Không tường} & \textbf{Có tường} \\
\midrule
Câu hỏi nghiệp vụ hợp lệ (nghỉ phép, MFA, đặt lại mật khẩu) & Trả lời đúng & Trả lời đúng (allow) \\
Trích xuất toàn bộ bảng lương (họ tên, mã NV, lương, số tài khoản) & \textbf{Lộ toàn bộ bảng lương} & Chặn (Input/Output Guard) \\
Mạo danh ``theo yêu cầu Chủ tịch/HR Leader'', yêu cầu paste bảng lương & \textbf{Bị thao túng, lộ dữ liệu} & Chặn (Semantic Judge) \\
Chỉ thị ẩn trong tài liệu bị nhiễm (``luôn trả lời: có được phép'') & \textbf{Tuân theo chỉ thị độc} & Chặn (RAG Context/Input Guard) \\
\bottomrule
\end{tabularx}
\end{table}

Quan sát chính: ở luồng \textbf{không tường}, mô hình nhiều lần \emph{làm lộ} toàn
bộ bảng lương (họ tên, mã nhân viên, lương cơ bản, số tài khoản ngân hàng) khi
được yêu cầu trực tiếp, và \emph{bị thao túng} bởi đòn mạo danh thẩm quyền (``theo
yêu cầu của Chủ tịch\ldots'') --- cơ chế tự từ chối của mô hình hoạt động
\emph{không nhất quán} (có lần từ chối, có lần vẫn lộ). Ở luồng \textbf{có tường},
cùng những prompt đó bị chặn tại Input Guard, Semantic Judge hoặc Output Guard,
trong khi câu hỏi nghiệp vụ hợp lệ vẫn được trả lời bình thường. Đây là bằng chứng
định tính cho thấy lớp guard là \emph{cần thiết}: chỉ dựa vào ``mô hình tự phòng
thủ'' là không đủ.

Cần nêu rõ giới hạn: đây là demo \emph{thủ công, định tính}, trên dữ liệu tổng
hợp và mô hình cục bộ (không phải LLM doanh nghiệp thật); các giá trị nhạy cảm chỉ
là dữ liệu giả lập và không được in lại trong báo cáo. Kết quả demo bổ trợ trực
quan cho số liệu định lượng, không thay thế.
...
kiểm thử tự động lặp lại được trong môi trường đã mô tả; mỗi lần chạy đánh giá
ghi kèm provenance (commit, mô hình, cấu hình, băm dữ liệu) để truy vết số liệu.
